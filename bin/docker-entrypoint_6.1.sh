...
if [ -e $CONFIGDIR/gunicorn_openerp.cfg.tmpl ]; then
  dockerize -template $CONFIGDIR/gunicorn_openerp.cfg.tmpl:$CONFIGDIR/gunicorn_openerp.cfg
fi
...
if [ -z "$(pip list --format=columns | grep "/opt/odoo/src")" ]; then
  ...
  # openerp.egg-info directory *inside /opt/odoo/src*. So when we run a container
  ...
  echo '/opt/odoo/src/openerp.egg-info is missing, probably because the directory is a volume.'
  echo 'Running pip install -e /opt/odoo/src to restore openerp.egg-info'
  ...
  chown -R odoo: /opt/odoo/src/openerp.egg-info
fi
...
BASE_CMD=$(basename $1)
if [ "$BASE_CMD" = "openerp-server" ] ; then
  ...
  if [ -z "$MIGRATE" -o "$MIGRATE" = True ]; then
    # NOTE: Marabunta seems to fail so set -e MIGRATE=False
