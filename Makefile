...
test:
	...
	mkdir $(TMP)/.cachedb
	# migration: standard
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -e LOAD_DB_CACHE="false" odoo odoo --stop-after-init
	# runmigration: create the dump
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -v $(TMP)/.cachedb:/.cachedb -e CREATE_DB_CACHE="true" odoo runmigration
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) odoo dropdb odoodb
	# runmigration: use dump
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -v $(TMP)/.cachedb:/.cachedb -e LOAD_DB_CACHE="true" odoo runmigration
	cd $(TMP) && docker-compose -f test-compose.yml down
	...
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) odoo dropdb odoodb
	# runmigration: ceil
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -v $(TMP)/.cachedb:/.cachedb -e LOAD_DB_CACHE="true" -e MIG_LOAD_VERSION_CEIL="9.0.1" odoo runmigration
	# runtests: standard
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -e LOAD_DB_CACHE="false" -e CREATE_DB_CACHE="false" odoo runtests
	## runtests: create the dump
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -v $(TMP)/.cachedb:/.cachedb -e CREATE_DB_CACHE="true" -e SUBS_MD5=testcache odoo runtests
	## runtests: use dump
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) -v $(TMP)/.cachedb:/.cachedb -e LOAD_DB_CACHE="true" -e SUBS_MD5=testcache  odoo runtests
	cd $(TMP) && docker-compose -f test-compose.yml down
	...
	cat $(TMP)/odoo/Dockerfile
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) odoo odoo --stop-after-init
	cd $(TMP) && docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=$(shell id -u) odoo runtests
	cd $(TMP) && docker-compose -f test-compose.yml down
